...
global opFile_NetworkAnalysis
opFile_NetworkAnalysis = None
def add_for_network_analysis(node1, node2List):
    global opFile_NetworkAnalysis
    
    if opFile_NetworkAnalysis is None:
        opFile_NetworkAnalysis = open('C:\\Users\\anand\\Google Drive\\Class - Knowledge Graph\\Project\\DATA\\tableau\\opFile_NetworkAnalysis.txt','wb')
    
    for node2 in node2List:
        opFile_NetworkAnalysis.write(node1+' '+node2+linesep)
...
#        if opJl['linkedentity'] == []: continue
        ...
        if not( 'Skills' in jsonObj and jsonObj['Skills'] != None): continue
        
        add_for_network_analysis(opJl['doc_id'], jsonObj['Skills'])
